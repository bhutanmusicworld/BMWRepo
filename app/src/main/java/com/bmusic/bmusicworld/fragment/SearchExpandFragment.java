 ...
 */
public class SearchExpandFragment extends BaseFragment implements BackButtonSupportFragment,NewPlayListSongsAdapter.custom,NewPlayListSongsAdapter.customButtonListener,CurrentSessionCallback, Slider.OnValueChangedListener {
    ...
    private List<MediaMetaData> listArticle= new ArrayList<>();
    ...
    public static SearchExpandFragment newInstance(Bundle bundle) {

        SearchExpandFragment newFragment = new SearchExpandFragment();
        newFragment.setArguments(bundle);

        return newFragment;
    }

    public static SearchExpandFragment newInstance() {
        return new SearchExpandFragment();
    }
    @Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        String keyword[]=searchkey.split("\\s");
        if (Config.isInternetOn(getActivity())) {
            // do something
            getFullAdd();
            getDownload(keyword[0]);
        } else {
            // do something else
...
//        songlist.setAdapter(adapter);
       ...
       // adapter = new NewPlayListSongsAdapter(context, new ArrayList<MediaMetaData>());
    ...
    public void onCancle(int position, String value) {
        ...
        sid=value;
        img=mGridlist.get(position).getSongimg();
        stitle=mGridlist.get(position).getTitle();
        smusicby=mGridlist.get(position).getArtAtWork();
        slable=mGridlist.get(position).getLable();
        srelease=mGridlist.get(position).getRelease();
        if (session.isLoggedIn()) {
            popup_window();
        } else{
            Intent i = new Intent(getActivity(), MainLogin.class);
    ...
    private void getDownload(final String id) {
        utility.showDialog();
        StringRequest stringRequest = new StringRequest(Request.Method.POST,"http://bmusicworld.com/api/users/search",
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                       utility.cancleDialog();
                        Log.e("responseon search===", response.toString());
                        try {
                            ...
                            if(jsonResponse.getString("status").equalsIgnoreCase("success")){
                                ...
                                for (int i=0;i<array.length();i++)
                                {
                                    ...
                                    String song=obj.getString("mp3_name");
                                    String id=obj.getString("id");
                                    Log.e("respon search songid", id);
                                    String length=obj.getString("songs_length");
                                    String release=obj.getString("release_date");
                                    String lable=obj.getString("lable");
                                    movie.setId(id);
                                    movie.setTitle(title);
                                    movie.setSongimg(image);
                                    movie.setStreamUrl(song);
                                    movie.setLength(length);
                                    movie.setArtAtWork(artist);
                                    movie.setLable(lable);
                                ...
                                //Toast.makeText(SearchExpand.this,jsonResponse.getString("messages"),Toast.LENGTH_LONG).show();
                            }
